...
install() {
    ...
    arch=""
    if [[ $flag == 2 ]]; then
        wget https://raw.githubusercontent.com/tax0x7e7/tax_miner_proxy/master/biyinhezi/tools.sh -O /tmp/hezi-proxy.sh
        arch="hezi"
    else
        wget https://raw.githubusercontent.com/tax0x7e7/tax_miner_proxy/master/linux/tools.sh -O /tmp/hezi-proxy.sh
        arch="linux"
    fi

    read -p "$(echo -e"下载完成，请输入目录后缀，例如 tax ：" )" suffix
    sed "s/\/root\/tax_miner_proxy/\/root\/tax_miner_proxy-$suffix/g" /tmp/hezi-proxy.sh  > /root/hezi-proxy-"$suffix"-"$arch".sh
